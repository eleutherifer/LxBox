...
- изменены наборы конечных точек для WARP,  
  например, в метро Wi-Fi на портах 443 и 8443 HTTP/3 (QUIC, MASQUE UDP) не работает, а на остальных портах на HTTP/3 не работают сейчас например SNI gosuslugi.ru, lenta.ru, mos.ru, ozon.ru, rzd.ru, yandex.ru, а из зарубежных SNI вообще работают только deepseek.com и pupi.org,  
